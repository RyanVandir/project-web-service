...
public class OrderController {
    ...
    public ResponseEntity<List<Order>> findAll() {
        return ResponseEntity.ok().body(orderUseCasePort.findAll());
    }

    @DeleteMapping("{id}")
    public ResponseEntity<Order> deleteById(@PathVariable Integer id) {
        return ResponseEntity.ok().body(orderUseCasePort.deleteById(id));
    }

    @GetMapping("{id}")
    public ResponseEntity<Order> findById(@PathVariable Integer id) {
        return ResponseEntity.ok().body(orderUseCasePort.findById(id));
    }
}
